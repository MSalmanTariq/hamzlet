...
, c.reduce=function(a, b, c) {
    ...
    g>f;
    ++f)a.hasOwnProperty(f)&&(e?d=b(d, a[f], f, a): (d=a[f], e=!0));
    return d;
}
...
var e=a("__browserify_process"), f=a("util"), g=a("_shims"), h=/^(\/?|)([\s\S]*?)((?: \. {
    1, 2;
}
|[^\/]+?|)(\.[^.\/]*|))(?: [\/]*)$/, i=function(a) {
...
;
    c.resolve=function() {
    for(var a="", b=!1, c=arguments.length-1;
    c>=-1&&!b;
    c--) {
...
), !b).join("/"), (b?"/": "")+a||"."}, c.normalize=function(a) {
...
a=c.resolve(a).substr(1), b=c.resolve(b).substr(1);
    for(var e=d(a.split("/")), f=d(b.split("/")), g=Math.min(e.length, f.length), h=g, i=0;
    g>i;
    i++)if(e[i]!==f[i]) {
    h=i;
    break;
...
function k(a, b, c, d, e) {
    for(var f=[], g=0, h=b.length;
    h>g;
    ++g)f.push(F(b, String(g))?l(a, b, c, d, String(g), !0): "");
...
, e=b.open||c.open||"<%", g=b.close||c.close||"%>", h=b.filename, i=b.compileDebug!==!1, j="";
    j+="var buf = [];
    ", !1!==b._with&&(j+="\nwith (locals ||  {
}
)  {
     (function() {
     "), j+="\n buf.push('";
    for(var k=1, l=!1, n=0, o=a.length;
    o>n;
    ++n) {
    var p=a[n];
...
var t=a.indexOf(g, n), u=a.substring(n, t), v=n, w=null, x=0;
    ...
    filename: z, _with:!1, open:e, close:g, compileDebug:i;
}
), j+="' + (function() {
    "+w+"}
)() + '", u=""}for(;
...
, d=l.readFileSync(b).toString(), e=p(d, c);
    a._compile("module.exports = "+e.toString()+";
    ", b);
}
...
, c.json=function(a) {
    ...
    c.escape=function(a) {
    return String(a).replace(/&(?!#?[a-zA-Z0-9]+;
    )/g, "&amp;
    ").replace(/</g, "&lt;
    ").replace(/>/g, "&gt;
    ").replace(/'/g, "&#39;
...
, e.call(this), f.call(this, a, b), (c=this.load())&&(d=c.items, h=c.settings, h&&(this._settings=h), d))for(j=0, i=d.length;
    i>j;
    j++)this.add(d[j]);
}
...
, a.currency=this.settings("currency_code"), h(e, a);
}
, c.prototype.total=function(a) {
    var b=0;
    return b+=this.subtotal(), b-=this.discount(), a=a|| {
}
, a.currency=this.settings("currency_code"), h(b, a);
}
...
, this.fire("destroy");
    ...
    name: "PPMiniCart", parent:"undefined"!=typeof document?document.body:null, action:"https://www.paypal.com/cgi-bin/webscr", target:"", duration:30, template:'<%var items = cart.items();
...
;
    var totalFormat =  {
     format:  true,  showCode: true }
;
    %><form method="post" class="<% if (!hasItems)  {
     %>minicart-empty<% }
 %>" action="https: //www.paypal.com/cgi-bin/webscr" target="google.com.pk">    <button type="button" class="minicart-closer">&times;
    </button>    <ul>        <% for (var i= 0,  idx = i + 1,  len = items.length;
     i < len;
     i++,  idx++)  {
     %>        <li class="minicart-item">            <div class="minicart-details-name">                <a class="minicart-name" href="<%= items[i].get("href") %>"><%= items[i].get("item_name") %></a>                <ul class="minicart-attributes">                    <% if (items[i].get("item_number"))  {
     %>                    <li>                        <%= items[i].get("item_number") %>                        <input type="hidden" name="item_number_<%= idx %>" value="<%= items[i].get("item_number") %>" />                    </li>                    <% }
 %>                    <% if (items[i].discount())  {
 ...
 %>                    <% for (var options = items[i].options(),  j = 0,  len2 = options.length;
     j < len2;
     j++)  {
     %>                        <li>                            <%= options[j].key %>:  <%= options[j].value %>                            <input type="hidden" name="on<%= j %>_<%= idx %>" value="<%= options[j].key %>" />                            <input type="hidden" name="os<%= j %>_<%= idx %>" value="<%= options[j].value %>" />                        </li>                    <% }
 %>                </ul>            </div>            <div class="minicart-details-quantity">                <input class="minicart-quantity" data-minicart-idx="<%= i %>" name="quantity_<%= idx %>" type="text" pattern="[0-9]*" value="<%= items[i].get("quantity") %>" autocomplete="off" />            </div>            <div class="minicart-details-remove">                <button type="button" class="minicart-remove" data-minicart-idx="<%= i %>">&times;
    </button>            </div>            <div class="minicart-details-price">                <span class="minicart-price"><%= items[i].total(priceFormat) %></span>            </div>            <input type="hidden" name="item_name_<%= idx %>" value="<%= items[i].get("item_name") %>" />            <input type="hidden" name="amount_<%= idx %>" value="<%= items[i].amount() %>" />            <input type="hidden" name="shipping_<%= idx %>" value="<%= items[i].get("shipping") %>" />            <input type="hidden" name="shipping2_<%= idx %>" value="<%= items[i].get("shipping2") %>" />        </li>        <% }
 %>    </ul>    <div class="minicart-footer">        <% if (hasItems)  {
     %>            <div class="minicart-subtotal">                <%= config.strings.subtotal %> <%= cart.total(totalFormat) %>            </div>            <button class="minicart-submit" type="button" onClick="cartSubmit()" data-minicart-alt="<%= config.strings.buttonAlt %>"><%- config.strings.button %>Check out</button>        <% }
 else  {
     %>            <p class="minicart-empty-text"><%= config.strings.empty %></p>        <% }
 %>    </div>    <input type="hidden" name="cmd" value="_cart" />    <input type="hidden" name="upload" value="1" />    <% for (var key in settings)  {
     %>        <input type="hidden" name="<%= key %>" value="<%= settings[key] %>" />    <% }
 %></form>', styles: '@keyframes pop-in  {
        0%  {
     opacity:  0;
     transform:  scale(0.1);
 }
...
', strings: {
    button: '', subtotal:"Subtotal:", discount:"Discount:", empty:"Your shopping cart is empty"}
};
...
, c.prototype.amount=function(a) {
    var b, c, e, f;
    if(!this._amount) {
    for(b=this.get("amount"), c=this.options(), f=0, e=c.length;
    e>f;
    f++)b+=c[f].amount;
    this._amount=b;
}
...
, remove: function(a, b, d) {
    ...
    for(g=0;
    h>g;
    g++)if(f=c[g], f[0]===a&&f[1]===b&&f[2]===d&&(e=f[3]))return a.removeEventListener(b, e, !1), c=c.slice(g), !0;
}
}: b.attachEvent? {
...
, remove: function(a, b, d) {
    ...
    for(g=0;
    h>g;
    g++)if(f=c[g], f[0]===a&&f[1]===b&&f[2]===d&&(e=f[3]))return a.detachEvent("on"+b, e), c=c.slice(g), !0;
}
}: void 0: {
    add: function() {
}
...
;
    for(e=0, f=g.length;
    f>e;
    e++)b=g[e], (d=c.getInputValue(b))&&(h[b.name]=d);
    return h;
}
, getInputValue: function(a) {
    ...
    var d;
    for(var e in b)d=b[e], d&&d.constructor===Object&&a[e]?c(a[e]|| {
}
, d): a[e]=d;
    return a;
}
...
, c.prototype.off=function(a, b) {
    ...
    if(e)for(c=0, d=e.length;
    d>c;
    c++)e[c]===b&&(e=e.splice(c, 1));
}
, c.prototype.fire=function(a) {
    var b, c, d, e, f=this._eventCache[a];
    if(f)for(b=0, c=f.length;
    c>b;
    b++)d=f[b][0], e=f[b][1]||this, "function"==typeof d&&d.apply(e, Array.prototype.slice.call(arguments, 1));
}
, b.exports=c;
},  {
}
...
, d.KEYUP_TIMEOUT));
    ...
    var a, b, f, g;
    for(a=document.getElementsByTagName("form"), f=0, g=a.length;
    g>f;
    f++)b=a[f], b.cmd&&d.COMMANDS[b.cmd.value]&&this.bind(b);
