...
<?php 
		...
		{  
			$Id=$_GET['CID'];
				$query="SELECT * FROM  `product`  JOIN `color` ON (product.Color_ID=color.Color_ID) JOIN size ON (product.S_ID=size.S_ID ) JOIN image ON (product.P_ID=image.P_ID  ) JOIN category ON (product.Category_ID=category.Category_ID )   WHERE (product.Category_ID=".$Id." and product.T_ID=1) order by product.P_Name";
		}
		...
		{  
			...
			$query="SELECT * FROM  `product`  JOIN `color` ON (product.Color_ID=color.Color_ID) JOIN size ON (product.S_ID=size.S_ID ) JOIN image ON (product.P_ID=image.P_ID  ) JOIN category ON (product.Category_ID=category.Category_ID ) WHERE (product.T_ID=1) order by product.P_Name";
